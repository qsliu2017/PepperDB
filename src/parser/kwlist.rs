//! parser/kwlist.h - the master SQL keyword list (X-macro database).
//!
//! In C this header is a pure X-macro "list" file with *deliberately* no include
//! guard (see the comment in the C source).  It is a flat sequence of
//! `PG_KEYWORD(name, value, category, is-bare-label)` invocations; each caller
//! `#define`s the `PG_KEYWORD` macro before `#include`-ing this file to extract
//! whatever projection it needs.  In PostgreSQL the two consumers are:
//!   * `gen_keywordlist.pl`, which precomputes the sorted keyword string +
//!     offsets + categories arrays (kwlist_d.h, ported in
//!     `crate::common::keywords`), and
//!   * `parser/gram.y`, which maps each `value` symbol to its Bison token.
//!
//! Rust has no textual-include / X-macro mechanism, so this module materializes
//! the raw projection: a `ScanKeywordEntry` const table carrying every entry's
//! textual `name`, the C `value` identifier (as a &str, since the Bison token
//! numbers are not known at this layer), its keyword `category`, and its
//! bare-label status.  The entries appear in ASCII order exactly as in the C
//! header (gen_keywordlist.pl requires this).
//!
//! The category constants are re-exported from `crate::common::keywords` (the
//! kwlist_d.h port) so there is a single source of truth; the bare-label
//! constants (which the .pl tool consumes but does not emit into kwlist_d.h)
//! are defined locally here.

use core::ffi::c_int;

// Keyword categories (parser/keywords.h).  Canonical definitions live in the
// kwlist_d.h port; re-export them so this raw-projection table can use them.
pub use crate::common::keywords::{
    COL_NAME_KEYWORD, RESERVED_KEYWORD, TYPE_FUNC_NAME_KEYWORD, UNRESERVED_KEYWORD,
};

// Bare-label status flags.  In the C header these are the bare symbols
// `BARE_LABEL` / `AS_LABEL` supplied as the 4th PG_KEYWORD argument; the
// PG_KEYWORD macro expansions in gram.y / common/keywords.c map them to a
// boolean (BARE_LABEL => can be a bare column label).  Defined locally here.
pub const BARE_LABEL: c_int = 1;
pub const AS_LABEL: c_int = 0;

/// One `PG_KEYWORD(name, value, category, is-bare-label)` entry from kwlist.h.
///
/// `value` is kept as the C token identifier string (e.g. `"ABORT_P"`); the
/// numeric Bison token value is assigned by gram.y and is not represented here.
#[repr(C)]
#[derive(Clone, Copy)]
pub struct ScanKeywordEntry {
    /// The keyword text (lower-case), 1st PG_KEYWORD argument.
    pub name: &'static str,
    /// The C token-value identifier, 2nd PG_KEYWORD argument.
    pub value: &'static str,
    /// Keyword category, 3rd PG_KEYWORD argument
    /// (UNRESERVED_KEYWORD / COL_NAME_KEYWORD / TYPE_FUNC_NAME_KEYWORD /
    /// RESERVED_KEYWORD).
    pub category: c_int,
    /// Bare-label status, 4th PG_KEYWORD argument (BARE_LABEL / AS_LABEL).
    pub is_bare_label: c_int,
}

/// Construct a `ScanKeywordEntry` from the four PG_KEYWORD arguments.  This is
/// the Rust stand-in for the C `PG_KEYWORD` X-macro; `value` is stringified.
macro_rules! PG_KEYWORD {
    ($name:literal, $value:ident, $category:ident, $label:ident) => {
        ScanKeywordEntry {
            name: $name,
...
/// The master keyword list, in ASCII order, verbatim from parser/kwlist.h.
pub static ScanKeywords: [ScanKeywordEntry; 494] = [
