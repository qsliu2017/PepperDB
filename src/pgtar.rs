...
pub enum tarHeaderOffset {
    NAME = 0,        // 100 byte string
    MODE = 100,      // 8 byte tar number, excludes S_IFMT
    UID = 108,       // 8 byte tar number
    GID = 116,       // 8 byte tar number
    SIZE = 124,      // 8 byte tar number
    MTIME = 136,     // 12 byte tar number
    CHECKSUM = 148,  // 8 byte tar number
    TYPEFLAG = 156,  // 1 byte file type
    LINKNAME = 157,  // 100 byte string
    MAGIC = 257,     // "ustar" + NUL
    VERSION = 263,   // "00"
    UNAME = 265,     // 32 byte string
    GNAME = 297,     // 32 byte string
    DEVMAJOR = 329,  // 8 byte tar number
    DEVMINOR = 337,  // 8 byte tar number
    PREFIX = 345,    // 155 byte string
}
...
pub enum tarFileType {
    PLAIN = b'0' as isize,
    PLAIN_OLD = b'\0' as isize, // backwards compatibility
    SYMLINK = b'2' as isize,
    DIRECTORY = b'5' as isize,
    PAX_EXTENDED = b'x' as isize,
    PAX_EXTENDED_GLOBAL = b'g' as isize,
}
