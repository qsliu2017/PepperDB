//! Snowball Swedish (ISO_8859_1) stemmer.
//!
//! 1:1 translation of the generated
//! `src/backend/snowball/libstemmer/stem_ISO_8859_1_swedish.c` (Snowball 2.2.0),
...
//! `skip_*_utf8` helpers, and the high character 0xF6 ('o-umlaut') is a single
//! raw byte rather than the two-byte UTF-8 sequence.
//!
...
use crate::snowball::utilities::{
    find_among_b, in_grouping, in_grouping_b, out_grouping, slice_del, slice_from_s,
};
...
static S_2_4: [symbol; 3] = [0xF6, b's', b't'];
...
unsafe fn r_mark_regions(z: *mut SN_env) -> c_int {
    *(*z).I.offset(1) = (*z).l;
    ...
    if out_grouping(z, G_V.as_ptr(), 97, 246, 1) < 0 {
        return 0;
    }

    {
        let ret = in_grouping(z, G_V.as_ptr(), 97, 246, 1);
        if ret < 0 {
            return 0;
        }
        (*z).c += ret;
...
unsafe fn r_main_suffix(z: *mut SN_env) -> c_int {
    ...
    match among_var {
        ...
        2 => {
            if in_grouping_b(z, G_S_ENDING.as_ptr(), 98, 121, 0) != 0 {
                return 0;
            }
            let ret = slice_del(z);
            if ret < 0 {
...
unsafe fn r_other_suffix(z: *mut SN_env) -> c_int {
    ...
    match among_var {
        ...
        2 => {
            if in_grouping_b(z, G_OST_ENDING.as_ptr(), 105, 118, 0) != 0 {
                return 0;
            }
...
#[no_mangle]
pub unsafe extern "C" fn swedish_ISO_8859_1_stem(z: *mut SN_env) -> c_int {
    {
        let c1 = (*z).c;
        {
...
#[no_mangle]
pub unsafe extern "C" fn swedish_ISO_8859_1_create_env() -> *mut SN_env {
    SN_create_env(0, 2)
}

#[no_mangle]
pub unsafe extern "C" fn swedish_ISO_8859_1_close_env(z: *mut SN_env) {
    SN_close_env(z, 0)
}
...
mod tests {
    ...
    fn root_unchanged() {
        unsafe {
            // "katt" - the consonant-pair "tt" sits at the very start so
            // nothing is removed; too short to enter R1 with a main suffix.
            let w = b"katt";
    ...
    fn idempotent() {
        unsafe {
            ...
            let once2 = stem(b"f\xf6rl\xf6sningar");
