//! Danish Snowball stemmer (UTF-8).
//!
//! 1:1 translation of the generated
//! `src/backend/snowball/libstemmer/stem_UTF_8_danish.c` (Snowball 2.2.0),
//! merged with its header `stem_UTF_8_danish.h`. The runtime helpers come from
...
use crate::snowball::utilities::{
    eq_s_b, eq_v_b, find_among_b, in_grouping_b_U, in_grouping_U, out_grouping_U, skip_b_utf8,
    skip_utf8, slice_del, slice_from_s, slice_to,
};
...
static S_2_4: [symbol; 5] = [b'l', 0xC3, 0xB8, b's', b't'];
...
static A_2: [among; 5] = [
    ...
    among { s_size: 5, s: S_2_4.as_ptr(), substring_i: -1, result: 2, function: None },
];
...
static S_2: [symbol; 4] = [b'l', 0xC3, 0xB8, b's'];
...
unsafe fn r_mark_regions(z: *mut SN_env) -> c_int {
    *(*z).I.offset(1) = (*z).l;
    ...
    {
        let ret = in_grouping_U(z, G_V.as_ptr(), 97, 248, 1);
        if ret < 0 {
            return 0;
        }
        (*z).c += ret;
...
unsafe fn r_main_suffix(z: *mut SN_env) -> c_int {
    ...
    match among_var {
        ...
        2 => {
            if in_grouping_b_U(z, G_S_ENDING.as_ptr(), 97, 229, 0) != 0 {
                return 0;
            }
            let ret = slice_del(z);
            if ret < 0 {
...
unsafe fn r_undouble(z: *mut SN_env) -> c_int {
    {
        ...
        (*z).lb = *(*z).I.offset(1);
        (*z).ket = (*z).c;
        if in_grouping_b_U(z, G_C.as_ptr(), 98, 122, 0) != 0 {
            (*z).lb = mlimit1;
            return 0;
        }
        (*z).bra = (*z).c;
...
#[no_mangle]
pub unsafe extern "C" fn danish_UTF_8_stem(z: *mut SN_env) -> c_int {
    {
        let c1 = (*z).c;
        {
...
#[no_mangle]
pub unsafe extern "C" fn danish_UTF_8_create_env() -> *mut SN_env {
    SN_create_env(1, 2)
}

#[no_mangle]
pub unsafe extern "C" fn danish_UTF_8_close_env(z: *mut SN_env) {
    SN_close_env(z, 1)
}
...
mod tests {
    ...
    unsafe fn stem(word: &[u8]) -> Vec<u8> {
        let z = danish_UTF_8_create_env();
        assert!(!z.is_null());
        let rc = SN_set_current(z, word.len() as c_int, word.as_ptr());
        assert!(rc >= 0);
        let rc = danish_UTF_8_stem(z);
    ...
    // The " heds" / "hed" suffix family collapses; result must be non-empty.
    #[test]
    fn suffix_stripped_nonempty() {
        unsafe {
            let r = stem(b"sandheder");
            assert!(!r.is_empty());
            // longer input cannot grow.
