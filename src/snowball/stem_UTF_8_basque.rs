//! Basque Snowball stemmer (UTF-8).
//!
//! 1:1 translation of the generated
//! `src/backend/snowball/libstemmer/stem_UTF_8_basque.c` (Snowball 2.2.0),
//! merged with its header `stem_UTF_8_basque.h`. The runtime helpers come from
...
use crate::snowball::utilities::{
    find_among_b, in_grouping_U, out_grouping_U, skip_utf8, slice_del, slice_from_s,
};
...
static S_1_107: [symbol; 4] = [0xC3, 0xB1, b'o', b'a'];
...
static S_1_204: [symbol; 3] = [0xC3, 0xB1, b'i'];
...
static S_1_261: [symbol; 3] = [0xC3, 0xB1, b'o'];
...
static A_1: [among; 295] = [
    ...
    among { s_size: 4, s: S_1_107.as_ptr(), substring_i: -1, result: 1, function: None },
    ...
    among { s_size: 3, s: S_1_204.as_ptr(), substring_i: -1, result: 1, function: None },
    ...
    among { s_size: 3, s: S_1_261.as_ptr(), substring_i: -1, result: 1, function: None },
        ...
        'lab0: {
            {
                ...
                'lab1: {
                    'lab2: {
                        if in_grouping_U(z, G_V.as_ptr(), 97, 117, 0) != 0 {
                            break 'lab2;
                        }
                        {
                            let c3 = (*z).c;
                            'lab3: {
                                'lab4: {
                                    if out_grouping_U(z, G_V.as_ptr(), 97, 117, 0) != 0 {
                                        break 'lab4;
                                    }
...
unsafe fn r_aditzak(z: *mut SN_env) -> c_int {
    let among_var;
    (*z).ket = (*z).c;
    if (*z).c - 1 <= (*z).lb
...
unsafe fn r_izenak(z: *mut SN_env) -> c_int {
    let among_var;
    (*z).ket = (*z).c;
    if (*z).c <= (*z).lb
        || (*(*z).p.offset(((*z).c - 1) as isize) as c_int >> 5) != 3
...
unsafe fn r_adjetiboak(z: *mut SN_env) -> c_int {
    let among_var;
    (*z).ket = (*z).c;
    if (*z).c - 1 <= (*z).lb
        || (*(*z).p.offset(((*z).c - 1) as isize) as c_int >> 5) != 3
...
#[no_mangle]
pub unsafe extern "C" fn basque_UTF_8_stem(z: *mut SN_env) -> c_int {
    {
...
#[no_mangle]
pub unsafe extern "C" fn basque_UTF_8_create_env() -> *mut SN_env {
    SN_create_env(0, 3)
}

#[no_mangle]
pub unsafe extern "C" fn basque_UTF_8_close_env(z: *mut SN_env) {
    SN_close_env(z, 0)
}
...
mod tests {
    ...
    fn idempotent() {
        unsafe {
            for w in [&b"etxea"[..], &b"liburua"[..], &b"mendia"[..]] {
                let once = stem(w);
                let twice = stem(&once);
