//! Porter Snowball stemmer (ISO-8859-1, single-byte).
//!
//! 1:1 translation of the generated
//! `src/backend/snowball/libstemmer/stem_ISO_8859_1_porter.c` (Snowball 2.2.0),
//! merged with its header. The runtime helpers come from
//! `crate::snowball::api` and `crate::snowball::utilities`. The algorithm is pure
//! ASCII, but the generated source targets the single-byte ISO-8859-1 runtime, so
//! this port uses the non-`_U` grouping helpers and plain byte advance/retreat.

use crate::prelude::*;

use crate::snowball::api::{among, symbol, SN_close_env, SN_create_env, SN_env, SIZE};
use crate::snowball::utilities::{
    find_among, find_among_b, in_grouping, insert_s, out_grouping, out_grouping_b, slice_del,
    slice_from_s,
};
...
unsafe fn r_shortv(z: *mut SN_env) -> c_int {
    if out_grouping_b(z, G_V_WXY.as_ptr(), 89, 121, 0) != 0 {
        return 0;
    }
    if in_grouping(z, G_V.as_ptr(), 97, 121, 0) != 0 {
        return 0;
    }
    if out_grouping_b(z, G_V.as_ptr(), 97, 121, 0) != 0 {
        return 0;
    }
    1
}
...
unsafe fn r_Step_1a(z: *mut SN_env) -> c_int {
    let among_var;
    (*z).ket = (*z).c;
...
unsafe fn r_Step_1b(z: *mut SN_env) -> c_int {
    let mut among_var;
    (*z).ket = (*z).c;
    if (*z).c - 1 <= (*z).lb
        ...
        2 => {
            {
                let m_test1 = (*z).l - (*z).c;
                {
                    let ret = out_grouping_b(z, G_V.as_ptr(), 97, 121, 1);
                    if ret < 0 {
                        return 0;
                    }
                    (*z).c -= ret;
            ...
            }
            {
                ...
                if (*z).c - 1 <= (*z).lb
                    ...
                    || (68514004 >> (*(*z).p.offset(((*z).c - 1) as isize) as c_int & 0x1f)) & 1
                        == 0
                {
                    among_var = 3;
                } else {
...
unsafe fn r_Step_1c(z: *mut SN_env) -> c_int {
    (*z).ket = (*z).c;
    {
        let m1 = (*z).l - (*z).c;
        'lab0: {
...
#[no_mangle]
pub unsafe extern "C" fn porter_ISO_8859_1_stem(z: *mut SN_env) -> c_int {
    *(*z).I.offset(2) = 0;
    {
        let c1 = (*z).c;
        'lab0: {
    ...
    *(*z).I.offset(0) = (*z).l;
    {
        ...
        'lab4: {
            {
                let ret = out_grouping(z, G_V.as_ptr(), 97, 121, 1);
                if ret < 0 {
                    break 'lab4;
                }
                (*z).c += ret;
            ...
            {
                let ret = in_grouping(z, G_V.as_ptr(), 97, 121, 1);
                if ret < 0 {
                    break 'lab4;
                }
                (*z).c += ret;
...
#[no_mangle]
pub unsafe extern "C" fn porter_ISO_8859_1_create_env() -> *mut SN_env {
    SN_create_env(0, 3)
}

#[no_mangle]
pub unsafe extern "C" fn porter_ISO_8859_1_close_env(z: *mut SN_env) {
    SN_close_env(z, 0)
}
...
mod tests {
    ...
    fn short_root_unchanged() {
        unsafe {
            assert_eq!(stem(b"cat"), b"cat".to_vec());
        }
    }

    // Idempotence: stemming a stem yields the same stem.
    //
    // NOTE: the *original* Porter algorithm (this file) is famously NOT idempotent
    // for every word - e.g. "agreed" -> "agre" -> "agr" (a second pass strips the
    // residual final "e" via Step_5a). That residual-stripping is faithful to the
    // C source (porter2/"english" later fixed such cases). So the words below are
    // restricted to ones the original Porter genuinely leaves fixed on re-stemming.
    #[test]
    fn idempotent() {
        unsafe {
            for w in [
    ...
    // The classic Porter example: "national" -> "nation".
    #[test]
    fn suffix_stripped_nonempty() {
        unsafe {
            let r = stem(b"national");
            assert!(!r.is_empty());
            assert!(r.len() <= "national".len());
        }
    }
}
