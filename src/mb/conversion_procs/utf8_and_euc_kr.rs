...
// TODO(pg-port): generated radix map from src/backend/utils/mb/Unicode/euc_kr_to_utf8.map - tables not transcribed
const EUC_KR_TO_UNICODE_TREE: pg_mb_radix_tree = pg_mb_radix_tree {
    chars16: null(),
    chars32: null(),
    b1root: 0,
    b1_lower: 0,
...
// TODO(pg-port): generated radix map from src/backend/utils/mb/Unicode/utf8_to_euc_kr.map - tables not transcribed
const EUC_KR_FROM_UNICODE_TREE: pg_mb_radix_tree = pg_mb_radix_tree {
    chars16: null(),
    chars32: null(),
    b1root: 0,
    b1_lower: 0,
 ...
 * ----------
 */
pub unsafe fn euc_kr_to_utf8(fcinfo: FunctionCallInfo) -> Datum {
...
pub unsafe fn utf8_to_euc_kr(fcinfo: FunctionCallInfo) -> Datum {
    let src = PG_GETARG_CSTRING!(fcinfo, 2) as *const c_uchar;
    let dest = PG_GETARG_CSTRING!(fcinfo, 3) as *mut c_uchar;
    let len: c_int = PG_GETARG_INT32!(fcinfo, 4);
    let no_error: bool = PG_GETARG_BOOL!(fcinfo, 5);
