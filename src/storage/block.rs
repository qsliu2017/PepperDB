...
const _: () = assert!(core::mem::offset_of!(BlockIdData, hi) == 0);
const _: () = assert!(core::mem::offset_of!(BlockIdData, lo) == 2);
...
impl BlockIdData {
    ...
    pub const fn set(&mut self, block_number: BlockNumber) {
        self.hi = (block_number >> 16) as u16;
        self.lo = (block_number & 0xffff) as u16;
    }
    ...
    pub const fn block_number(&self) -> BlockNumber {
        ((self.hi as BlockNumber) << 16) | (self.lo as BlockNumber)
    }
}
