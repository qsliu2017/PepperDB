//! rangetypes_selfuncs.rs
//!   Functions for selectivity estimation of range operators
//!
//! Translated 1:1 from postgres/src/backend/utils/adt/rangetypes_selfuncs.c
//!
...
/* range op OIDs (catalog/pg_operator.h via pg_known_oids.rs) */
use crate::catalog::pg_known_oids::{
    OID_RANGE_CONTAINED_OP, OID_RANGE_CONTAINS_ELEM_OP, OID_RANGE_CONTAINS_OP,
    OID_RANGE_ELEM_CONTAINED_OP, OID_RANGE_LESS_OP,
};
...
use crate::utils::adt::rangetypes::{
    range_cmp_bounds, range_deserialize, range_get_typcache, range_serialize, DatumGetRangeTypeP,
    RangeBound, RangeIsEmpty, RangeType, TypeCacheEntry,
};
...
/* DEFAULT_RANGE_INEQ_SEL / DEFAULT_INEQ_SEL (selfuncs.h, not exported). */
...
/*
 * Range operator OIDs that are not yet present in pg_known_oids.rs.  These
 * b-tree-style range comparison operator OIDs come from pg_operator.dat.
 */
// TODO(pg-port): real OID_RANGE_LESS_EQUAL_OP lives in catalog/pg_operator.h
const OID_RANGE_LESS_EQUAL_OP: Oid = 3885;
// TODO(pg-port): real OID_RANGE_GREATER_EQUAL_OP lives in catalog/pg_operator.h
const OID_RANGE_GREATER_EQUAL_OP: Oid = 3886;
// TODO(pg-port): real OID_RANGE_GREATER_OP lives in catalog/pg_operator.h
const OID_RANGE_GREATER_OP: Oid = 3887;
// TODO(pg-port): real OID_RANGE_OVERLAP_OP lives in catalog/pg_operator.h
const OID_RANGE_OVERLAP_OP: Oid = 3888;
// TODO(pg-port): real OID_RANGE_LEFT_OP lives in catalog/pg_operator.h
const OID_RANGE_LEFT_OP: Oid = 3893;
// TODO(pg-port): real OID_RANGE_RIGHT_OP lives in catalog/pg_operator.h
const OID_RANGE_RIGHT_OP: Oid = 3894;
// TODO(pg-port): real OID_RANGE_OVERLAPS_LEFT_OP lives in catalog/pg_operator.h
const OID_RANGE_OVERLAPS_LEFT_OP: Oid = 3895;
// TODO(pg-port): real OID_RANGE_OVERLAPS_RIGHT_OP lives in catalog/pg_operator.h
 ...
 */
unsafe fn default_range_selectivity(operator: Oid) -> f64 {
    match operator {
 ...
 */
pub unsafe fn rangesel(fcinfo: FunctionCallInfo) -> Datum {
    ...
    if operator == OID_RANGE_CONTAINS_ELEM_OP {
        ...
        if (*(other as *mut Const)).consttype == (*(*typcache).rngelemtype).type_id {
            ...
            upper.lower = false;
            constrange =
                range_serialize(typcache, &raw mut lower, &raw mut upper, false, null_mut());
        }
     ...
     */
    if !constrange.is_null() {
        selec = calc_rangesel(typcache, &raw mut vardata, constrange, operator);
    } else {
...
unsafe fn calc_hist_selectivity(
    ...
    if OidIsValid((*typcache).rng_subdiff_finfo.fn_oid)
        && !statistic_proc_security_check(vardata, (*typcache).rng_subdiff_finfo.fn_oid)
    {
        return -1.0;
    }
    ...
    nhist = hslot.nvalues;
    hist_lower =
        palloc(std::mem::size_of::<RangeBound>() * nhist as usize) as *mut RangeBound;
    hist_upper =
        palloc(std::mem::size_of::<RangeBound>() * nhist as usize) as *mut RangeBound;
    i = 0;
    while i < nhist {
    ...
    match operator {
        OID_RANGE_LESS_OP => {
             ...
             */
            hist_selec =
                calc_hist_selectivity_scalar(typcache, &raw const const_lower, hist_lower, nhist, false);
        }

        OID_RANGE_LESS_EQUAL_OP => {
            hist_selec =
                calc_hist_selectivity_scalar(typcache, &raw const const_lower, hist_lower, nhist, true);
        }

        OID_RANGE_GREATER_OP => {
            hist_selec =
                1.0 - calc_hist_selectivity_scalar(typcache, &raw const const_lower, hist_lower, nhist, false);
        }

        OID_RANGE_GREATER_EQUAL_OP => {
            hist_selec =
                1.0 - calc_hist_selectivity_scalar(typcache, &raw const const_lower, hist_lower, nhist, true);
        }

        OID_RANGE_LEFT_OP => {
            /* var << const when upper(var) < lower(const) */
            hist_selec =
                calc_hist_selectivity_scalar(typcache, &raw const const_lower, hist_upper, nhist, false);
        }

        OID_RANGE_RIGHT_OP => {
            /* var >> const when lower(var) > upper(const) */
            hist_selec =
                1.0 - calc_hist_selectivity_scalar(typcache, &raw const const_upper, hist_lower, nhist, true);
        }
        ...
        OID_RANGE_OVERLAPS_LEFT_OP => {
            ...
            hist_selec =
                calc_hist_selectivity_scalar(typcache, &raw const const_upper, hist_upper, nhist, true);
        }
        ...
        OID_RANGE_OVERLAP_OP | OID_RANGE_CONTAINS_ELEM_OP => {
            ...
            let mut hs =
                calc_hist_selectivity_scalar(typcache, &raw const const_lower, hist_upper, nhist, false);
            hs += 1.0
                - calc_hist_selectivity_scalar(typcache, &raw const const_upper, hist_lower, nhist, true);
            hist_selec = 1.0 - hs;
        }
        ...
        _ => {
            elog!(ERROR, "unknown range operator {}", operator);
...
unsafe fn calc_hist_selectivity_scalar(
    ...
    if index >= 0 && index < hist_nvalues - 1 {
        selec += get_position(typcache, constbound, hist.add(index as usize), hist.add((index + 1) as usize))
