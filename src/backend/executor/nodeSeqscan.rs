...
#![allow(
    ...
    reason = "rules.md s5: the scan's per-query state (the scan/result TupleTableSlot, the raw Relation handle, the HeapScanDescData) is !Send and task-confined -- one backend task drives the scan, never sent across tasks. Same contract as the table AM scan futures."
)]
...
use crate::backend::access::heap::heapam::{HeapScanDescData, SendRelation, SendPtr};
...
pub struct SeqScanRun {
    pub state: Box<SeqScanState>,
    /// the open scan `Relation` (PG keeps it in `ss_currentRelation`, whose field
    /// is the opaque forward placeholder here; read from the executor relation
    /// registry at init).
    pub relation: Relation,
    /// the open heap scan descriptor, created on the first `ExecSeqScan`.
    pub scan: Option<Box<HeapScanDescData>>,
    /// the query snapshot (`estate->es_snapshot`) the scan runs under, captured at
    /// init (the C scan reads es_snapshot off the EState; the Rust `ScanState` has
    /// no estate-snapshot field reachable here, so stash it on the run-state).
    pub snapshot: Arc<crate::utils::snapshot::SnapshotData>,
}
...
pub fn exec_init_seq_scan(node: &SeqScan, estate: &mut EState, eflags: i32) -> Box<SeqScanRun> {
    let _ = eflags;
    crate::assert!(
    ...
    let ss = ScanState {
        ps,
        ss_current_relation: crate::nodes::execnodes::Relation,
...
async fn seq_next(shared: &Arc<SharedState>, run: &mut SeqScanRun) -> bool {
    // table_beginscan on the first call (needs the query snapshot, available now).
    if run.scan.is_none() {
        let relation = run.relation;
        let snapshot = Arc::clone(&run.snapshot);
        run.scan = Some(table_beginscan(SendPtr(relation), snapshot));
    }
    let scan = run
        .scan
        .as_mut()
    ...
    // SAFETY: `tuple` points into scan.ctup (valid until the next getnext), whose
    // t_data aliases the pinned page; deform reads the header + data bytes.
...
pub fn exec_end_seq_scan(shared: &Arc<SharedState>, run: &mut SeqScanRun) {
...
pub fn exec_rescan_seq_scan(shared: &Arc<SharedState>, run: &mut SeqScanRun) {
    if let Some(scan) = run.scan.as_mut() {
        table_endscan(shared, scan);
    }
    run.scan = None;
...
/// index `rti`, read from the executor's per-task relation registry (the
/// es_relations equivalent; see execMain). The heavyweight-lock relation_open path
/// grows with the relcache lock-manager wiring.
fn exec_get_range_table_relation(_estate: &EState, rti: crate::nodes::primnodes::Index) -> Relation {
    crate::assert!(rti > 0);
    let relation = crate::backend::executor::execMain::exec_relation_for_rti(rti)
        .unwrap_or_else(|| unimplemented!("ExecGetRangeTableRelation: scan relation not registered for RTI"));
    crate::assert!(!relation.is_null(), "scan relation not opened (registry slot null)");
    relation
}
...
mod tests {
    ...
    use crate::backend::executor::execMain::{with_exec_relations, standard_executor_end, standard_executor_start, standard_executor_run};
    ...
    fn make_relation(locator: RelFileLocator, tupdesc: TupleDesc) -> *mut RelationData {
        ...
        let rel = RelationData {
            ...
            rd_pkindex: crate::postgres_ext::Oid(0),
            rd_ispkdeferrable: false,
            rd_replidindex: crate::postgres_ext::Oid(0),
            rd_statlist: Vec::new(),
            rd_attrsvalid: false,
            rd_keyattr: None,
            rd_pkattr: None,
            ...
            rd_amhandler: crate::postgres_ext::Oid(2),
            ...
            rd_toastoid: crate::postgres_ext::Oid(0),
    ...
    async fn insert_row(shared: &Arc<SharedState>, relation: *mut RelationData, vals: &[i32]) {
    ...
    #[allow(deprecated)]
    fn query_desc(stmt: PlannedStmt, snap: crate::utils::snapshot::Snapshot, dest: Box<dyn DestReceiver>) -> QueryDesc {
        QueryDesc {
    ...
    async fn run_seqscan(
        ...
        let mut qd = query_desc(stmt, snap, dest);

        with_exec_relations(vec![(1usize, relation)], Box::pin(async {
            standard_executor_start(&mut qd, 0);
    ...
    async fn modifytable_insert_then_seqscan_sees_it() {
        ...
        Box::pin(in_all_scopes(shared.clone(), |shared| async move {
            ...
            // Run the INSERT.
            let snap = current_snapshot(&shared);
            let mut qd = query_desc(insert_stmt, snap, Box::new(CollectingDest { sink: Arc::new(Mutex::new(Collected::default())) }));
            with_exec_relations(vec![(1usize, rel)], Box::pin(async {
                standard_executor_start(&mut qd, 0);
                standard_executor_run(Some(&shared), &mut qd, ScanDirection::Forward, 0).await;
                standard_executor_end(Some(&shared), &mut qd);
            }))
